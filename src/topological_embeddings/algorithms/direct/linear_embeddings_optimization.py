...
def get_direct_embeddings(graph, embeddings_size, l_rate=0.01, iterations=1000, loss_function=rmrse_loss, kappa=1.0):
    ...
    def optimize_embeddings():
        embeddings = torch.rand(number_of_nodes, embeddings_size).to(torch_device)
        embeddings.requires_grad_()
        optimizer = torch.optim.Adam([embeddings], lr=l_rate)
